...
static void sigint_handler(int sig) {
    ...
    if (shared_global) {
        ...
        printf("\nGardener 2: SIGINT caught, stopping...\n");
        fflush(stdout);
    }
}
...
static void work_on_cell(SharedData *sh, int i, int j, int gardener_id) {
    ...
    if (sh->cell_state[i][j] == CELL_EMPTY) {
        ...
        printf("Gardener 2 processed cell (%d,%d)\n", i, j);
        fflush(stdout);
        ...
        printf("Gardener 2 skipped cell (%d,%d), state=%d\n", i, j, sh->cell_state[i][j]);
        fflush(stdout);
        sleep_us(sh->pass_time_us);
    }
...
// Основной цикл обхода сада садовником 2.
static void gardener2_loop(SharedData *sh) {
    int M = sh->M;
    int N = sh->N;
...
// Точка входа процесса садовника 2.
int main(void) {
    int shm_fd = shm_open(SHM_NAME, O_RDWR, 0666);
    if (shm_fd == -1) {
        perror("shm_open (gardener2)");
        fprintf(stderr, "Gardener 2: run garden_ctl first.\n");
        return 1;
    }

    shared = mmap(NULL, sizeof(SharedData), PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 0);
    if (shared == MAP_FAILED) {
        perror("mmap (gardener2)");
        close(shm_fd);
        return 1;
    }
    ...
    if (sigaction(SIGINT, &sa, NULL) == -1) {
        perror("sigaction (gardener2)");
    }

    printf("Gardener 2: waiting for controller to initialize...\n");
    fflush(stdout);
    ...
    if (shared->stop) {
        printf("Gardener 2: stop flag set before start, exiting.\n");
    ...
    for (int i = 0; i < M; ++i) {
        for (int j = 0; j < N; ++j) {
            ...
            if (cell_sem[i][j] == SEM_FAILED) {
                perror("sem_open cell (gardener2)");
                munmap(shared, sizeof(SharedData));
                close(shm_fd);
                return 1;
            }
    ...
    printf("Gardener 2: started.\n");
    fflush(stdout);

    gardener2_loop(shared);
